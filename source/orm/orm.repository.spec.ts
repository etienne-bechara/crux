...
export const OrmRepositorySpec = ({
	type,
	port,
	user,
}: { type: 'mysql' | 'postgresql'; port: number; user: string }): void => {
			...
			it('should create entities with nested one-to-many relations', async () => {
				await userRepository.create([
					{
						...
						age: 50,
						metadata: [{ key: 'ONE_TO_MANY_METADATA_KEY_1', value: 'ONE_TO_MANY_METADATA_VALUE_1' }],
					},
					{
				...
				expect(
					user2.metadata
						...
						.map((m) => m.key)
						.sort(),
				).toStrictEqual(['ONE_TO_MANY_METADATA_KEY_2', 'ONE_TO_MANY_METADATA_KEY_3']);
				expect(
					user2.metadata
						.toArray()
						.map((m) => m.value)
						.sort(),
				).toStrictEqual(['ONE_TO_MANY_METADATA_VALUE_2', 'ONE_TO_MANY_METADATA_VALUE_3']);
			});
				...
				await userRepository.create([
					{
						name: 'SERIALIZE_1',
						age: 50,
						metadata: [{ key: 'SERIALIZE_METADATA_KEY_1', value: 'SERIALIZE_METADATA_VALUE_1' }],
					},
				]);
			...
			it('should upsert entities with one-to-many relationships accordingly', async () => {
				await userRepository.create([
					{
						...
						age: 10,
						metadata: [{ key: 'UPSERT_1M_METADATA_KEY_1', value: 'UPSERT_1M_METADATA_VALUE_1' }],
					},
					{
				...
				expect(
					user1.metadata
						...
						.map((m) => m.key)
						.sort(),
				).toStrictEqual(['UPSERT_1M_METADATA_KEY_4', 'UPSERT_1M_METADATA_KEY_5']);
				...
				expect(
					user2.metadata
						...
						.map((m) => m.key)
						.sort(),
				).toStrictEqual(['UPSERT_1M_METADATA_KEY_1', 'UPSERT_1M_METADATA_KEY_2']);
			...
			it('should upsert entities with many-to-many relationships accordingly', async () => {
				...
				expect(
					order1.products
						...
						.map((p) => p.title)
						.sort(),
				).toStrictEqual(['UPSERT_MN_PRODUCT_2']);
				...
				expect(
					order2.products
						...
						.map((p) => p.title)
						.sort(),
				).toStrictEqual(['UPSERT_MN_PRODUCT_2', 'UPSERT_MN_PRODUCT_3']);
				...
				expect(
					order3.products
						...
						.map((p) => p.title)
						.sort(),
				).toStrictEqual(['UPSERT_MN_PRODUCT_1', 'UPSERT_MN_PRODUCT_2', 'UPSERT_MN_PRODUCT_3']);
			});
		});
